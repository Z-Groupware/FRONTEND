 ...
 * ⚠️ **배너(`NotificationBanner`·`PasswordChangeBanner`)는 여기서 포털로 안 받는다**(2026-08-14
 *    변경). 셸(`(shell)/layout.tsx`)이 자기 트리 안에서 `{children}` 다음에 배너 스택을 두어
 *    DOM 순서(상단바 컨트롤 → 배너)를 맞춘다 — 도메인마다 이 헤더가 다시 마운트돼도 배너가
 *    안 사라지게 하려는 것이다.
 */
