...
import { HeaderBannerAnchor } from "@/features/shell/header-banner-slot";
 ...
 * ⚠️ **`HeaderBannerAnchor`가 바로 뒤에 붙는다.** 셸이 배너(`NotificationBanner`·
 *    `PasswordChangeBanner`)를 여기로 포털해 넣는다(`header-banner-slot.tsx`) — 배너가
 *    상단바보다 시각적으로도 DOM 순서로도 항상 아래에 오게 하려는 자리다. CSS `order`로
 *    시각 순서만 바꾸면 키보드 탭·스크린리더 순서가 어긋난다(적대적 리뷰, 2026-08-14) —
 *    그래서 실제 DOM 위치를 옮기는 포털을 쓴다.
 */
