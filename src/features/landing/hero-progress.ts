 ...
 * ⚠️ **둘째 구간 끝을 0.86 너머로 밀지 마라**(2026-08-14에 0.95로 밀었다가 되돌렸다).
 *    `finaleAt`이 0.86부터 시작하므로 겹치는 순간 **부서진 채로 완성 연출이 돈다**(커지고
 *    림라이트가 켜지는데 조각은 아직 흩어져 있다). 게다가 이 값은 `--hero-shatter`로 CSS에도
 *    나가서 `.hero-z-dark`가 `(1 - shatter*0.35)`만큼 진하기를 깎는데, 그러면 **가장 진해야 할
 *    완성 순간에 로고가 제일 흐려진다**(측정: 0.86에서 0.59, 제 값은 0.90).
 *    정말 더 오래 벌어져 있게 하려면 `finaleAt`의 시작점도 **같이** 뒤로 옮겨야 한다.
 */
