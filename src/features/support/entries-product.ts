...
export const PRODUCT_ENTRIES: readonly FaqEntry[] = [
  ...
  },
  {
    ...
    // ⚠️ 없는 걸 있는 것처럼 흘리지 않는다 — 구독 상태(ACTIVE·CANCELING·UNPAID·EXPIRED)로만 답한다(§요금제·§정직성)
    answer:
      "***무료 요금제는 없습니다.*** 워크스페이스는 **구독 상태가 ACTIVE**일 때만 열립니다. 온보딩 마지막 단계에서 첫 결제가 통과하면 ACTIVE가 되고, 그때 열립니다.\n\n이후 **UNPAID**(미결제)나 **EXPIRED**(만료)가 되면 다시 잠기고, **CANCELING**(해지 예정)이면 다음 결제일까지는 그대로 이용하실 수 있습니다.\n\n등급을 나눠 위 등급을 파는 방식이 아니라 **기능은 처음부터 전부 열려 있습니다.** 인원도 제한이 없어 몇 명을 초대해도 기본료는 같습니다.",
    links: [{ label: "요금제 보기", href: "/plans" }],
  },
  {
    id: "plan-change",
