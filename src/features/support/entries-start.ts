...
export const START_ENTRIES: readonly FaqEntry[] = [
  {
    ...
    answer:
      "**기업 등록 신청**을 남겨 주세요.\n\n검토 후 담당자 메일로 **기업 코드**를 보내 드립니다. 그 코드로 로그인하면 워크스페이스가 열립니다.",
  ...
  },
  {
    ...
    /*
      ⚠️ `팀 체계`를 여기 남긴다. 이 말은 **두 항목에 다 걸리는 게 맞다** — 구조가 어떻게
         생겼는지(`org-structure`)와 그걸 언제 만드는지(여기)는 다른 질문이고, 물어본
         사람이 어느 쪽인지는 말만 봐서 모른다. 둘 다 걸리면 화면이 되묻는다(§match).
      ⚠️ 뺐더니 `팀 체계는 어떻게 만드나요`가 `org-structure` 하나로만 걸렸다. 그 항목은
         구조를 설명할 뿐 **만드는 방법을 말하지 않는데**, 후보가 하나라 되묻지도 않고
         그대로 답이 나갔다.
    */
    keywords: [
      "온보딩",
      ...
      "부서 체계",
      /*
        ⚠️ **`팀 만들기`·`팀 추가`처럼 만드는 말도 넣는다.** 용어를 부서에서 팀으로 바꾸면서
           구용어(`부서 만들기`)로는 걸리고 새 용어(`팀 만들기`)로는 **후보가 0개**였다 —
           `팀`은 한 글자라 문턱(`MIN_SCORE`)을 혼자 못 넘고, `팀 체계`는 `체계`가 있어야
           걸리기 때문이다(적대적 검토 #163).
      */
      "팀 만들기",
      "팀 추가",
  ...
  },
  {
    ...
    /*
      ...
      ⚠️ **화면에 "무료"라고 쓰지 않는다**(CLAUDE.md §요금제, 2026-08-13 적대적 검증에서
         잡혔다). "무료 체험 기간은 따로 없습니다"도 "무료"라는 글자가 화면에 남아, 훑어보는
         사람은 그 단어만 보고 무료 체험이 있는 것처럼 오해할 수 있다 — "무료" 없이 같은
         내용을 전한다. keywords의 "무료 체험"은 검색어 매칭용이라 화면에 안 뜬다.
    */
    answer:
      "**체험 기간은 따로 없습니다.** 기업 등록이 승인되면 결제를 마쳐야 워크스페이스가 열립니다.\n\n대신 요금이 회사당 하나라 인원을 몇 명으로 시작하든 금액은 같습니다.\n\n화면을 먼저 보고 싶으시면 아래 주소로 문의해 주세요.",
  ...
  },
  {
    ...
    answer:
      "회사에서 워크스페이스를 맡을 분이 하시면 됩니다. 신청서에 적은 담당자가 **첫 관리자**가 됩니다.\n\n팀·직급 설정과 사원 초대를 맡게 되니, 그 일을 책임질 분으로 정해 주세요.",
    links: [{ label: "기업 등록 신청", href: "/register" }],
  },
];
