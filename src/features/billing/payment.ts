...
const FAILURE_MESSAGE: Record<string, string> = {
  /*
    ⚠️ **결제사가 아니라 우리 서버가 내는 코드다.** 지금 BE가 `pay`에서 실제로 내는 사유는
       이것 하나뿐이다(`BillingCommandService.pay`). 여기 없으면 기본 문구로 떨어져
       **"카드사에서 결제를 처리하지 못했습니다"** 가 뜨는데, 카드를 건 적도 없는 사람에게
       카드사를 탓하는 말이라 원인을 반대로 알린다 — 사람은 카드사에 전화를 걸게 된다(§정직성).
    ⚠️ **다음에 뭘 하라고 적지 않는다.** 결제 화면(온보딩 4단계·게이트)에는 카드를 걸 자리가
       아직 없어서, `등록해 주세요`라고 쓰면 있지도 않은 길을 가리킨다 — 사실만 적는다.
  */
  NO_PAYMENT_METHOD: "등록된 결제 수단이 없습니다",
