...
describe("회의실 예약 생성 (Owner 개설 = 프로젝트 회의)", () => {
  ...
  it("폼이 조작돼 존재하지 않는 회의실 id가 오면 막는다", async () => {
    const result = await createRoomReservationAction(
      { errors: {} },
      form({ ...VALID_ENTRIES, roomId: "room-does-not-exist", date: "2026-08-21" }),
    );
  ...
  it("폼이 조작돼 존재하지 않는 프로젝트 id가 오면 막는다", async () => {
    const result = await createRoomReservationAction(
      { errors: {} },
      form({ ...VALID_ENTRIES, projectId: "999", date: "2026-08-21" }),
    );
  ...
  it("폼이 조작돼 존재하지 않는 참석자 id가 오면 막는다", async () => {
    const result = await createRoomReservationAction(
      { errors: {} },
      form({ ...VALID_ENTRIES, date: "2026-08-21" }, [9999]),
    );
  ...
  it("폼이 조작돼 참석자 값이 숫자가 아니면 막는다", async () => {
    const data = new FormData();
    for (const [key, value] of Object.entries({ ...VALID_ENTRIES, date: "2026-08-21" })) {
...
describe("회의실 예약 생성 (Leader 개설 = 팀 액션 회의)", () => {
  ...
  it("상위 팀 액션 없이 제출하면 막는다", async () => {
    const result = await createRoomReservationAction(
      { errors: {} },
      form({ ...VALID_ENTRIES, date: "2026-08-24" }),
    );
  ...
  it("자기 팀의 진짜 팀 액션이면 통과하고, 회의에도 그대로 담긴다", async () => {
    ...
    const result = await createRoomReservationAction(
      { errors: {} },
      form({ ...VALID_ENTRIES, date: "2026-08-24", parentTeamActionId: "1" }),
    );
  ...
  it("다른 팀 소속 팀 액션 id를 끼워 넣으면 막는다(폼 조작 방어)", async () => {
    ...
    const result = await createRoomReservationAction(
      { errors: {} },
      form({ ...VALID_ENTRIES, date: "2026-08-24", parentTeamActionId: "3" }),
    );
  ...
  it("자기 팀이 아닌 사람을 참석자로 끼워 넣으면 막는다(2026-08-13 범위 강제)", async () => {
    ...
    const result = await createRoomReservationAction(
      { errors: {} },
      form({ ...VALID_ENTRIES, date: "2026-08-24", parentTeamActionId: "1" }, [2, 7]),
    );
  ...
  it("다른 프로젝트 소속 팀 액션 id를 끼워 넣으면 막는다", async () => {
    ...
    const result = await createRoomReservationAction(
      { errors: {} },
      form({ ...VALID_ENTRIES, date: "2026-08-24", parentTeamActionId: "7" }),
    );
