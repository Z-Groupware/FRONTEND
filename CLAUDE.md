...
- 값 목록은 **`constants/`에 정의**한다. 문서에 옮겨 적지 않는다(바뀌면 두 벌이 어긋난다).
  ...
  매퍼에서 조용히 틀린다.
- ⚠️ **나간 사람은 목록에 남는다**(2026-08-05 확정). 회원 상태 `RESIGNED`(퇴사)는 **화면에 보이는 상태**다 —
  그 사람이 남긴 회의·액션·인수인계의 출처라, 이름이 사라지면 추적이 끊긴다.
  소프트 딜리트(`DELETED`)는 상태가 아니라 **목록에서 빠지는 일**이라 상태 목록에 넣지 않고
  매퍼가 `isVisibleMemberStatus`로 거른다.
  ⚠️ 신청·승인 흐름은 **오프보딩**(`HANDOVER_TYPE.OFFBOARDING`), 끝난 뒤의 사람 상태는 **퇴사**(`MEMBER_STATUS.RESIGNED`)다.
  같은 일의 두 쪽이라 라벨을 섞어 쓰면 화면이 다른 말을 한다.
