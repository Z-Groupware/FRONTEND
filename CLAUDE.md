...
- ⚠️ **계층 비교로 짜지 않는다.** BE 권한도 선형 계층이 아니라
...
- 값 목록은 **`constants/`에 정의**한다. 문서에 옮겨 적지 않는다(바뀌면 두 벌이 어긋난다).
  ⚠️ 프론트 정본은 `constants/domain.ts`다. **BE enum과 이름·값을 맞춰 둔다** — 다르면 매퍼에서
  조용히 어긋난다. 값은 **담당자 도메인 문서로 확인**한 뒤 고친다(§연동 검증).
...
- ⚠️ ERD·API 스펙 **미확정** → 지금은 목 기준. 담당자 도메인 문서를 받으면 **매퍼만** 고친다.
  ⚠️ 목 shape을 마음대로 지어내지 않는다 — 문서를 받기 전이라도 아는 만큼은 맞춰 둔다.
...
- 응답 봉투는 성공 `{ httpStatus, message, data }` · 실패 `{ errorCode, message, ... }`로 **가정**하고 짰다
  (⚠️ 낡은 문서 기준 · 담당자 문서로 재확인). 모양이 바뀌어도 **봉투를 벗기는 건 매퍼 한 곳**이라
  컴포넌트는 손대지 않는다 — 봉투를 컴포넌트가 알면 그때는 전 화면을 고쳐야 한다.
...
- [ ] **ERD·API 스펙** — 한 번 돈 BE 코드 컨벤션 문서는 **최신이 아니다**(2026-08-05 확인).
      값·정책은 **각 도메인 담당자의 문서**를 받아 맞춘다(§연동 검증).
...
- [ ] **캡처 파이프라인** — 옵션 A(STT=요약 입력 / 음성=아카이브 / 메모=즉시 저장)로 이야기됐으나
      담당자 문서로 확인 필요. 잔여: 청크 통합본 저장 주체 · socket.io 중계 유지 · STT 폴백
- [ ] 결제 PG — Toss로 이야기됐으나 미확정. 이음매는 `billing/payment-method.ts`에 갈라 뒀다
