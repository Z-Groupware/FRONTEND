...
- 값 목록은 **`constants/`에 정의**한다. 문서에 옮겨 적지 않는다(바뀌면 두 벌이 어긋난다).
  ⚠️ 값은 **담당자 도메인 문서로 확인**한 뒤 고친다(§연동 검증). BE enum과 이름이 어긋나면
  매퍼에서 조용히 틀린다.
...
- **① 담당자 도메인 문서를 받아** 정책·Enum·에러코드를 맞춘다. BE는 도메인별로 담당자가
  문서를 관리한다 — 우리 화면이 닿는 도메인의 문서를 요청해 정합성부터 맞춘다.
...
- 응답 봉투 모양은 **아직 모른다.** 벗기는 일은 **매퍼 한 곳**에서 하고 컴포넌트는 봉투를 모른다 —
  컴포넌트가 봉투를 알면 모양이 바뀔 때 전 화면을 고쳐야 한다.
...
- ⚠️ **AI 검토 화면은 있다**(`/app/meeting/:id/review`, 2026-08-05 팀 확정). 담당자·마감일을
  고칠 자리가 없으면 잘못 뽑힌 액션이 그대로 하달된다 — **[액션 분배 확정]을 눌러야 생성된다.**
...
> **BE가 이미 페이지 단위로 준다.** 프론트가 정하는 건 **다음 페이지를 언제 부르느냐**뿐이다.

- **기준은 스크롤이다.** 1페이지(10건)를 다 내리면 그때 2페이지를 이어 붙인다.
  ❌ 전체를 받아 `slice`로 나누지 않는다 — 화면만 잘릴 뿐 10만 건을 다 받아 온다.
- **트리거는 `IntersectionObserver` 감시자**(목록 끝의 빈 요소). `scroll` 이벤트로 위치를
  계산하지 않는다 — 스크롤은 초당 수십 번 뛰어서 INP를 갉아먹는다(§성능).
- **첫 페이지는 서버가 렌더**하고, 2페이지부터 클라이언트가 이어 붙인다.
  첫 화면까지 `use client`로 만들면 조회 전체가 클라이언트로 넘어간다(§핵심 4원칙 1).
- **[더 보기] 버튼을 함께 둔다.** 스크롤로만 불러오면 키보드·스크린리더 사용자는 다음
  페이지에 갈 방법이 없다(§a11y — DnD 보드와 같은 이유다).
- **이어 붙일 때 `id`로 중복을 거른다.** 보는 사이에 새 항목이 맨 앞에 들어오면 행이 한 칸
  밀려 **같은 줄이 두 번** 나온다. 붙이는 자리에서 걸러야 화면이 거짓말을 안 한다.
- **전체 건수를 머리에 적는다**(`전체 128건`). 끝이 안 보이는 목록은 얼마나 남았는지 알 수 없다.
- **3상태는 그대로다** — 첫 로딩은 스켈레톤, 이어 붙일 때는 목록 아래 작은 로딩,
  실패하면 조용히 멈추지 말고 **[다시 시도]** 를 띄운다(§정직성).
- 수백 행을 한 화면에 쌓아야 하면 그때 가상화를 검토한다.
...
- [ ] **ERD·API 스펙** (BE 협의 전) — 값·정책은 **각 도메인 담당자의 문서**를 받아 맞춘다(§연동 검증)
