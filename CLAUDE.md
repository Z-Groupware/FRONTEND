...
- 값 목록은 **`constants/`에 정의**한다. 문서에 옮겨 적지 않는다(바뀌면 두 벌이 어긋난다).
  ...
  매퍼에서 조용히 틀린다.
- ⚠️ **나간 사람은 오프보딩 최종 승인 즉시 소프트 딜리트된다**(2026-08-15 정정 — "RESIGNED가 목록에 남는다"였던 이전 결정을 BE와 재협의해 뒤집음). BE `offboard()`가 승인 시점에 바로 `deleted_at`을 세팅해 그 사람은 **구성원 목록·조직도 같은 명부 조회**(`/manage/members`·`/app/people` 등)에서 완전히 빠진다 — `MEMBER_STATUS.RESIGNED`는 감사 흔적용 내부 값일 뿐, 화면·매퍼가 볼 일이 없다(`isVisibleMemberStatus`로 거를 대상 자체가 안 내려온다).
  ⚠️ **명부 조회와 기록 조회는 범위가 다르다.** 위 "모든 조회에서 빠진다"는 명부 한정이다 — 그 사람이 참석자·개설자였던 회의, 담당자였던 액션은 각 기록이 이름을 스냅샷으로 들고 있어 계속 보이고, 그 자리에 "퇴사자" 표시만 붙는다. 회사 전체 명부에서 그 사람을 검색·나열할 수 있게 두지 않을 뿐이다.
  ⚠️ **팀장급 퇴사로 액션이 잠시 담당자를 잃는 구간**(귀속 대기, WORKFLOW §7)은 "팀장이 공석입니다"로 표시한다. 새 팀장에게 귀속되면 **이 "공석" 표시만** 사라진다 — 과거 회의·액션에 남은 "퇴사자" 표시는 귀속 이후에도 그대로 남는다(둘은 서로 다른 표시다).
  ⚠️ 신청·승인 흐름은 **오프보딩**(`HANDOVER_TYPE.OFFBOARDING`), 승인 뒤 남는 것은 "퇴사자 표시가 붙은 과거 기록들"뿐 — 명부에서 조회 가능한 "퇴사 상태의 사람"은 없다.
...
> 🎨 **화면 만들 때는 [`docs/DESIGN.md`](docs/DESIGN.md)를 먼저 훑는다.** 여기는 값이고,
> 거기는 **언제 어떤 값을 고르는지와 틀렸을 때 나는 일**이다 — 폭·표면·표 정렬·색·워딩을
> 만들기 전에 정하면 되돌릴 일이 없다.
>
...
- **다크:** 배경·셸 `#1A1715` · 카드 `#242120` · 보조 `#2E2A28` · 보더 `#33302D` · 텍스트 `#FAFAF9` · 보조텍스트 `#A8A29E`
  ...
  - **사이드바·상단바·본문은 `--background` 한 색**, 카드는 `--card`.
    ⚠️ **로그인 뒤 워크스페이스에는 점 그리드를 안 깐다**(2026-08-05 변경). 일하는 화면이라
    바탕이 조용해야 표·카드가 먼저 읽힌다. `.bg-dot-grid`는 **처음 만나는 화면**
    (랜딩·온보딩·결제)에만 남는다 — 유틸을 지우지 않는 건 그 화면들이 계속 쓰기 때문이다.
    ⚠️ 라이트는 `--background`·`--card`가 **둘 다 흰색**이다. 카드는 색이 아니라
    **얕은 그림자**로 띄운다(`.app-shell .bg-card`, 다크는 안 건다).
    ⚠️ **바탕을 회색·크림으로 밀지 않는다.** 세 번 해 봤고 각각 시멘트색·따로 노는 느낌·
    누런 종이로 읽혔다 — 밝은 색은 색조가 그대로 읽힌다(DECISIONS §셸 표면).
...
- **레이아웃:** 사이드바 220px + PageLayout 4종 — `list`(1440) · `detail`(1440 2컬럼) · `form`(960, 좌 네비 180px) · `centered`(560 세로중앙)
  ...
  - ⚠️ **`form`의 960은 좌 네비(180px)가 있을 때다.** 네비 없는 **단일 폼 한 장은 720**이다
    (마이페이지·공지 작성/수정). 네비 자리까지 늘리면 입력칸만 넓어져 읽고 쓰기가 나빠진다 —
    항목이 늘어 좌 네비가 생기는 날 960으로 올린다. **1440으로 올리지 않는다**(그건 목록·표용이다).
  - ⚠️ **읽는 글은 좁게 둔다.** 공지 상세는 바깥이 `list`(1440)이고 **글 카드만 720**이다.
    한 줄이 길면 눈이 다음 줄을 못 찾는다.
  - ⚠️ **새 숫자를 만들지 않는다.** 1080·1120·1144처럼 중간값이 늘면 화면마다 여백이 달라
    사이드바로 옮겨 다닐 때 본문 폭이 눈에 띄게 흔들린다(2026-08-05 정리).
...
- ⚠️ **[더 보기] 버튼은 안 둔다**(2026-08-10 폐기 — 이전엔 키보드·스크린리더 접근성을
  이유로 뒀으나, 스크롤 트리거 하나로 통일하기로 팀이 정했다). 실패했을 때의 [다시 시도]
  버튼만 남긴다 — 그건 수동 트리거가 필요한 별개의 경우다.
- **페이지 인덱스는 0-base다**(BE 표준 확정, 2026-08-10 — `page=0`이 첫 페이지).
...
- 대상: ①회의 요약(3줄·결정·액션) ②액션 분배 **둘뿐이다.**
  ⚠️ **"프로젝트 자동 매칭"은 없다**(2026-08-16 확정 — WORKFLOW.md에 한 번도 안 나오고 구현도 없다).
  프로젝트는 **사람이 필수로 고른다**(예약 모달·비대면 모달 둘 다 프로젝트 select가 필수) — 없는 걸
  적어 두면 나중에 누가 이걸 근거로 만든다(§도메인 상수와 같은 이유).
