...
- 값 목록은 **`constants/`에 정의**한다. 문서에 옮겨 적지 않는다(바뀌면 두 벌이 어긋난다).
  ...
  매퍼에서 조용히 틀린다.
- ⚠️ **나간 사람은 오프보딩 최종 승인 즉시 소프트 딜리트된다**(2026-08-15 정정 — "RESIGNED가 목록에 남는다"였던 이전 결정을 BE와 재협의해 뒤집음). BE `offboard()`가 승인 시점에 바로 `deleted_at`을 세팅해 그 사람은 이후 모든 조회에서 완전히 빠진다 — `MEMBER_STATUS.RESIGNED`는 감사 흔적용 내부 값일 뿐, 화면·매퍼가 볼 일이 없다(`isVisibleMemberStatus`로 거를 대상 자체가 안 내려온다).
  ⚠️ **추적은 "명부에 남기는 방식"이 아니라 "개별 기록에 표시하는 방식"으로 한다.** 그 사람이 참석자·개설자인 회의, 담당자였던 액션에는 각 기록 쪽에서 "퇴사자" 표시를 단다 — 회사 전체 명부에서 그 사람을 계속 조회 가능하게 두지 않는다.
  ⚠️ **팀장급 퇴사로 액션이 잠시 담당자를 잃는 구간**(귀속 대기, WORKFLOW §7)은 "팀장이 공석입니다"로 표시한다. 새 팀장에게 귀속되면 그 표시는 사라진다.
  ⚠️ 신청·승인 흐름은 **오프보딩**(`HANDOVER_TYPE.OFFBOARDING`), 승인 뒤 남는 것은 "퇴사자 표시가 붙은 과거 기록들"뿐 — 조회 가능한 "퇴사 상태의 사람"은 없다.
