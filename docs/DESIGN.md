...
> 🚨 **참고 문서가 아니다. 이대로 만든다.**
> 여기 적힌 골격·값·문구를 그대로 쓰면 기존 화면과 같은 생김새가 나온다. 새로 정하지 않는다 —
> 정하는 순간 사람마다 화면이 갈린다.
> 기준은 **구독**(`/manage/billing`)과 **저장소 관리**(`/manage/storage`)다.
>
> **화면을 새로 만들 때는 §1을 복사해서 시작한다.**
>
> | 문서                                     | 무엇이 있나                                |
> | ---------------------------------------- | ------------------------------------------ |
> | 여기                                     | **어떻게 만드는지** — 골격·값·규칙         |
> | [`WORKFLOW.md`](WORKFLOW.md)             | **무엇을 만드는지** — 화면별 요구사항·라벨 |
> | [`CONVENTIONS.md`](CONVENTIONS.md) §8·§9 | 토큰·폭 표 (정본)                          |
...
⚠️ **카드는 두 장이다.** 요약 → 내역. 세 장째가 필요하면 위 카드에 넣을 수 있는지 먼저 본다.
...
가장 자주 어긋나는 곳이다. **묶음을 통째로 가운데 두면 안 된다** — 안에 든 글자 길이가
다르면 그 줄만 밀린다.
...
- **새 숫자를 만들지 않는다.** 1080·1120·1144가 섞이면 화면마다 여백이 달라진다.
