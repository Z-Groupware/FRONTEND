...
- **조직도는 2계층**(부서 > 역할), 트리 구조로 처음부터 설계한다. ⚠️ 3계층에서 줄인 것이다(DECISIONS 2026-07-31).
  - **아랫단은 하위 부서가 아니라 역할이다.** 사원은 **부서에 소속되고**, 역할은 비워둘 수 있다(=`없음`, 예: 팀장).
  - **권한은 부서가 아니라 직급에서 온다.** LEADER 직급은 회사에 하나, 그 직급을 가진 사람은 부서마다 한 명이다.
  - **LEADER는 자기 부서 전체**를 관리한다(조회·초대·통계·중간승인 전부 그 부서의 모든 역할 포함).
