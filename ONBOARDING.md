...
1. **PR을 올리면 검사 4종(typecheck·lint·test·build)이 돈다.** 하나라도 빨간불이면 머지가 안 된다. (GitHub에는 `verify` 체크 1개로 보인다)
...
| **PR**            | 올리면 끝 | **검사 4종**이 돌고, 빨간불이면 머지 못 함                      |
| **`any` 사용**    | 경고만    | **에러 → 빌드 실패**                                            |
...
| 색상          | 하드코딩       | **디자인 토큰**(CSS 변수)으로 통일. **다크모드 전 페이지 적용**       |
...
- `verify` 체크가 **초록**이어야 함 (typecheck·lint·test·build 4종을 한 번에 돈다)
- **리뷰 1명 승인** 후 머지
- ⛔ **본인 PR 본인 머지 금지**
...
```
① 역할(Role)     : OWNER / ADMIN / LEADER / MEMBER / SYSTEM(확장)
