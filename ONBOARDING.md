...
8. [Claude Code로 작업할 때](#8-claude-code로-작업할-때)
9. [자주 묻는 것](#9-자주-묻는-것)
...
3. **규칙은 문서에 있고, Claude가 그 문서를 자동으로 읽는다.** 그래서 AI한테 매번 규칙을 설명 안 해도 된다.
...
```
CLAUDE.md              ← Claude가 매 세션 자동으로 읽는 규칙 (린)
...
git pull
git checkout -b feature/meeting-capture     # 또는 fix/...
```

- **base는 항상 `develop`** (`main`은 릴리즈용, 우리는 안 건드림)
- 형식: `feature/{도메인}-{기능}` · `fix/{도메인}-{내용}`
...
이유: 다크모드 값이 이미 정의돼 있어서, 토큰을 쓰면 **다크모드가 공짜로 따라온다.** 하드코딩하면 나중에 47화면을 다 고쳐야 한다.
...
| **CLAUDE.md**               | 직접 읽을 일은 적다 — **Claude가 자동으로 읽는다.** 규칙 요약본 |
...
## 8. Claude Code로 작업할 때

우리는 개발을 Claude로 많이 한다. 그래서 규칙을 **문서로** 만들어뒀다 — `CLAUDE.md`는 **매 세션 자동으로 읽힌다.** 매번 규칙을 설명할 필요가 없다.

### 지시할 때

- **한 번에 다 시키지 말고 나눈다:** ① 타입/스키마 → ② API 함수 → ③ 화면
  (UI 버그와 연동 버그가 섞이면 원인 찾기가 어렵다)
- **"다시 확인해" 같은 말은 넣지 않는다.** 알아서 검증하므로 중복이다.
- **범위를 명확히.** 안 시킨 리팩터까지 하는 경향이 있어서, 애매하면 "이것만" 이라고 못 박는다.

### 결과를 믿기 전에

- `npm run typecheck && npx eslint . && npm run build` — **AI가 "완료"라고 해도 이걸로 확인한다**
- 특히 **API 경로**를 지어내지 않았는지(`ep.*` 썼는지), **권한 2축**을 다 검사했는지 본다
